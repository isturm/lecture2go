...
<jsp:useBean id="videoSearchContainer" type="de.uhh.l2g.plugins.admin.videos.search.VideoSearchContainer"
             scope="request"/>
...
<div class="admin-video">
    <div class="edit-video-lable">${pageName}</div>
    <div class="d-flex flex-row-reverse">
        <div class="col-md-4">
                ...
                <c:if test="${permissionAdmin}">
                    ...
                    <aui:form action="${sortByCoordinator}" method="post">
                        <aui:select name="coordinatorId" label="" onChange="submit();">
                            <aui:option value="">
                                <liferay-ui:message key="select-coordinator"/>
                            </aui:option>
                            <c:forEach items="${coordinators}" var="item">
                                <aui:option
                                        value='${item.coordinatorId}'>${item.lastName}, ${item.firstName}</aui:option>
                            </c:forEach>
                        </aui:select>
                    </aui:form>
                </c:if>
                ...
                <c:if test="${permissionCoordinator || (permissionAdmin && coordinatorId>0)}">
                    ...
                    <aui:form action="${sortByProducer}" method="post">
                        <aui:select name="producerId" label="" onChange="submit();">
                            <aui:option value="">
                                <liferay-ui:message key="select-producer"/>
                            </aui:option>
            ...
            <c:if test="${producerId>0}">
                ...
                <a href="${addVideoURL}" class="add-link">
                    <span class="icon-plus"></span> <liferay-ui:message key="add-video"/>
                </a>
            </c:if>
        ...
        <div class="col-md-8">
            <liferay-ui:search-container-row className="de.uhh.l2g.plugins.model.Video" keyProperty="videoId"
                                             modelVar="video">
                ...
                <liferay-ui:search-container-column-text name="">
                    <div class="adminrow wide">
                        ...
                        <c:set var="creators"
                               value="<%=CreatorLocalServiceUtil.getCommaSeparatedLinkedCreatorsByVideoIdAndMaxCreators(video.getVideoId(), 100)%>"/>
                        ...
                        <div class="video-image-wrapper col-md-4" onClick="${onClick}">
                        ...
                        <div class="col-md-8">
                            <div class="video-label">
                                ...
                                <div class="video-edit-wrapper">
                                    ...
                                    <a href="${editURL}" title="<liferay-ui:message key='edit'/>"
                                       alt="<liferay-ui:message key='edit'/>">
                                        <span class="icon-pencil"></span>
                                    </a>

                                    <c:if test="${fn:length(vid.filename)>0}">
                                        <c:if test="${vid.openAccess==1}">
                                            <a href="${lockURL}" title="<liferay-ui:message key='lock-help'/>"
                                               alt="<liferay-ui:message key='lock-help'/>">
                                    <span class="icon-unlock" onclick="return confirm('<liferay-ui:message
                                            key="really-lock-question"/>')"></span>
                                            </a>
                                        </c:if>

                                        <c:if test="${vid.openAccess==0}">
                                            <a href="${unlockURL}" title="<liferay-ui:message key='unlock-help'/>"
                                               alt="<liferay-ui:message key='unlock-help'/>">
                                    <span class="icon-lock" onclick="return confirm('<liferay-ui:message
                                            key="really-unlock-question"/>')"></span>
                                            </a>
                                        </c:if>

                                        <c:if test="${vid.downloadLink==1}">
                                            <a href="${deactivateDowonloadURL}"
                                               title="<liferay-ui:message key='deaktivate-download-help'/>"
                                               alt="<liferay-ui:message key='deaktivate-download-help'/>">
                                    <span class="icon-download-alt"
                                          onclick="return confirm('<liferay-ui:message
                                                  key="really-deactivate-download-question"/>')"></span>
                                            </a>
                                        </c:if>

                                        <c:if test="${vid.downloadLink==0}">
                                            <a href="${activateDowonloadURL}"
                                               title="<liferay-ui:message key='aktivate-download-help'/>"
                                               alt="<liferay-ui:message key='aktivate-download-help'/>">
                                                <span class="icon-download"></span>
                                            </a>
                                        </c:if>

                                        <c:set var="segments"
                                               value="<%=SegmentLocalServiceUtil.getSegmentsByVideoId(video.getVideoId())%>"/>
                                        <c:if test="${segments.size()>0}">
                                            <a href="${segmentURL}"
                                               title="<liferay-ui:message key='comment-video-help'/>"
                                               alt="<liferay-ui:message key='comment-video-help'/>">
                            ...
                            <span class="icon-remove"
                                  onclick="return confirm('<liferay-ui:message key="really-delete-question"/>')"></span>
                                    </a>
                                </div>
                            </div>
