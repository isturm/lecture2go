...
<jsp:useBean id="video" type="de.uhh.l2g.plugins.model.Video"
	scope="request" />
<jsp:useBean id="relatedVideos"
...
<jsp:useBean id="videoInstitutions"
	...
	scope="request" />
<jsp:useBean id="videoMetadata" type="de.uhh.l2g.plugins.model.Metadata"
	scope="request" />
<jsp:useBean id="lectureseries"
	type="de.uhh.l2g.plugins.model.Lectureseries" scope="request" />
...
<c:set var="tags"
	value="<%=video.getTags().length() > 0 ? " - " + video.getTags() + " - " : ""%>" />
<c:set var="pageTitle"
...
<c:if test="${video.videoId>0}">
	<c:choose>
		<c:when test="${video.accessPermitted==1}">
			<c:if test="${video.openAccess==1 || video.openAccess==0}">
				...
				<c:choose>
					<c:when test="${video.lectureseriesId>0}">
						<c:set var="lec"
							value="<%=LectureseriesLocalServiceUtil.getLectureseries(video.getLectureseriesId())%>" />
						<c:set var="institutions"
							value="<%=InstitutionLocalServiceUtil.getByLectureseriesId(video.getLectureseriesId(),
						...
						<c:forEach var="j" begin="0" end="0">
							...
							<c:set var="pInst"
								value="<%=InstitutionLocalServiceUtil
												.getInstitution((Long) pageContext.getAttribute("pInstId"))%>" />
							<c:set var="rInstId" value="${pInst.parentId}" />
							<c:set var="rInst"
								value="<%=InstitutionLocalServiceUtil
												.getInstitution((Long) pageContext.getAttribute("rInstId"))%>" />
			...
			%>
			<c:set var="term"
				value="<%=TermLocalServiceUtil.getById((Long) pageContext.getAttribute("lTermId"))%>" />
			<c:choose>
			...
			<div class="row">
				<div class="col-md-7 col-12">
					<div id="main">
						<div class="${relatedVideos.size()>1 ? 'player' : 'player-wide'}">
								...
								<div class="license">
									<c:choose>
										<c:when test="${videoLicense.l2go==1}">
											<a href="/license-l2go"
												title="<liferay-ui:message
                               key='l2go-license-click-for-info'/>">
												<liferay-ui:message key="license" />: <liferay-ui:message
													key='l2go-license' />
											</a>
										</c:when>
										<c:otherwise>
											<a href="https://creativecommons.org/licenses/by-nc-sa/3.0/"
												title="<liferay-ui:message key='cc-license-click-for-info'/>"><liferay-ui:message
													key="license" />: <liferay-ui:message
													key='cy-nc-sa-license' /> </a>
										</c:otherwise>
									</c:choose>
								</div>
								<div class="views">
							...
							<div
								class="${relatedVideos.size()>1} ? 'meta-video-info': 'meta-video-info-wide'">
								<%
									try {
								%>
								...
								<div class="meta-title">
									<c:if test="${isCitation2Go}">
										<div class="c2go-title">
											<liferay-ui:message key='citation-of' />
										</div>
									</c:if>
									${video.title}
								</div>
								<div class="meta-creators">
									...
									<c:set var="creators"
										value="<%=CreatorLocalServiceUtil
								.getCommaSeparatedLinkedCreatorsByVideoIdAndMaxCreators(video.getVideoId(), 100)%>" />
									${creators}
								</div>
								<div class="meta-description-container">
									<div class="meta-description">
										${videoMetadata.description.trim().length() > 0 ? videoMetadata.description :lectureseries.longDesc}
									</div>
								</div>
							</div>

							<c:if
								test="${timeEnd==0 || timeEnd<timeStart || video.citation2go==0}">
								<div
									class="${relatedVideos.size()>1 ? 'metainfo' : 'metainfo-wide'}">
									<ul id="tabs" class="nav nav-tabs" data-tabs="tabs">
										<c:if test="${video.downloadLink==1}">
											<li class="nav-item" id="down">
											<a class="nav-link" href="#download"	data-toggle="tab"><liferay-ui:message key="download" /></a>
											</li>
										</c:if>
										<c:if test="${video.openAccess==1}">
											<li class="nav-item"><a class="nav-link active" href="#share" data-toggle="tab"><liferay-ui:message
														key="share" /></a></li>
										</c:if>
										<li class="nav-item"><a  class="nav-link" href="#support" data-toggle="tab"><liferay-ui:message
													key="support" /></a></li>
										<c:if test="${video.hasChapters}">
											<li class="nav-item"><a href="#chapters"
				...
				<c:if test="${relatedVideos.size()>1}">
					<div class="related col-md-5 col-12">
						<div class="related-lectureseries-name col-12">
							<liferay-ui:message key="lecture-series" />
							&nbsp;<a target="_blank" class="icon-small icon-rss"
								href="${video.mp4RssLink}"></a>
						</div>
						<ul class="ul-related">
							<c:forEach items="${relatedVideos}" var="vid">
								...
								<li class="videotile small related ${activeClass}"
									...
									<div class="metainfo-small related col-8">
										...
										<p class="creator-small2 related">${vid.linkedCreators}</p>
...
<c:otherwise>
	<c:if test="${video.accessPermitted==0}">
		...
		<aui:form action="${viewOpenAccessVideoURL2.toString()}" method="post">
			<aui:fieldset helpMessage="enter-password" column="true"
				label="l2go-video-password">
