...
@Component(immediate = true, property = { "javax.portlet.name=" + OpenAccessVideosPortletKeys.OPEN_ACCESS_VIDEOS,
		"mvc.command.name=/view/render/details" }, service = MVCRenderCommand.class)
public class ViewRenderDetails implements MVCRenderCommand {
	...
	public String render(RenderRequest renderRequest, RenderResponse renderResponse) throws PortletException {
		...
		try {
			...
			if (objectType.equals("v")) { // for video objects
				try {
					objectId = VideoLocalServiceUtil.getBySecureUrl(oid, true).getVideoId();
					is360Video = is360Video(objectId);
					secLink = true;
		...
		if (objectType.equals("l")) {
			try {
				...
				if (!secLink) {
					try {
						video = VideoLocalServiceUtil.getCurrentlyValidVideo(lectureseries.getPreviewVideoId());
					} catch (PortalException e1) {
					}
				} else {
					Long videoId = VideoLocalServiceUtil.getLatestClosedAccessVideoId(objectId, true);
					try {
						video = VideoLocalServiceUtil.getCurrentlyValidVideo(videoId);
					} catch (PortalException e1) {
					}
				}
			} catch (Exception e) {
			...
			try {
				video = VideoLocalServiceUtil.getCurrentlyValidVideo(objectId);
			} catch (PortalException e1) {
			}
		...
		if (objectExists) {
			...
			/*
			 * generate symbolic links in the download folder if the download is enabled and
			 * symbolic links are not yet existing. we currently check this on every video
			 * page view, as there may be files generated in the file system (like a
			 * generation of a mp3 by the postprocessing engine ) which do not trigger an
			 * event in lecture2go, so it could be handled there
			 */
			...
			try {
				...
				relatedVideos = VideoLocalServiceUtil
						.getByLectureseriesAndOpenaccess(lectureseries.getLectureseriesId(), os, true);
	...
	private boolean is360Video(Long objectId) {
		...
		mediaTypes.forEach(video_mediaType -> {
			try {
				if (MediaTypeLocalServiceUtil.getMediaType(video_mediaType.getMediaTypeId()).getMediaTypeName()
						.contains("360")) {
					is360Video.set(true);
				}
			} catch (PortalException e) {
