 ...
 * {@link de.uhh.l2g.plugins.service.LectureseriesLocalService} interface.
 *
 * <p>
...
public class LectureseriesLocalServiceImpl extends LectureseriesLocalServiceBaseImpl {
	...
	public List<Lectureseries> getFilteredByApprovedSemesterFacultyProducer(Integer approved, Long semester,
			Long facultyId, Long producerId, Long groupId, Long companyId) {
		List<Lectureseries> l = lectureseriesFinder.findFilteredByApprovedSemesterFacultyProducer(approved, semester,
				facultyId, producerId, groupId, companyId);
		try {
			l.isEmpty();// check
	...
	public Map<Term, List<Lectureseries>> getFilteredByApprovedSemesterFacultyProducerAsTreeMapSortedByTerm(
			Integer approved, Long semester, Long facultyId, Long producerId, Long groupId, Long companyId) {
		List<Lectureseries> l = getFilteredByApprovedSemesterFacultyProducer(approved, semester, facultyId, producerId,
				groupId, companyId);
		Map<Term, List<Lectureseries>> lectureseriesTreeMapSortedByTerm = new TreeMap<Term, List<Lectureseries>>(
				new Comparator<Term>() {
					@Override
...
//		Video_LectureseriesLocalServiceUtil.updateOpenAccessByVideo(video);
		...
		// lecture series
		Video latestOpenAccessVideo = VideoLocalServiceUtil
				.getLatestOpenAccessVideoForLectureseries(lectureseries.getLectureseriesId());
	...
	public void updatePreviewVideoOpenAccess(Lectureseries lectureseries) throws SystemException {
		List<Video> openAccessVideosOfLectureseries = VideoLocalServiceUtil
				.getByLectureseriesAndOpenaccess(lectureseries.getLectureseriesId(), 1, false);

		if (lectureseries.getVideoSort() == 1) {
			// sort ascending
			Collections.sort(openAccessVideosOfLectureseries, new Comparator<Video>() {
				@Override
				public int compare(Video v1, Video v2) {
					return v1.getGenerationDate().compareTo(v2.getGenerationDate());
				}
			});
		} else {
			// sort descending
		...
		while (itAll.hasNext()) {
			...
			try {
				...
				if (v.getUploadDate() != null)
					l.setLatestVideoUploadDate(v.getUploadDate());
				else {
	...
	public void updateCategoryForLectureseries(Long lectureseriesId, Long newCategoryId)
		...
		Lectureseries_CategoryLocalServiceUtil.updateCategoryByLectureseriesAndCategory(
				lectureseries.getLectureseriesId(), lectureseries.getCategoryId(), newCategoryId);
		...
		while (vli.hasNext()) {
			Video v = vli.next();
			Video_CategoryLocalServiceUtil.updateCategoryByVideoAndCategory(v.getVideoId(),
					lectureseries.getCategoryId(), newCategoryId);
		}
	...
	public List<Lectureseries> getByKeyWordsAndCompanyId(String keywords, Long companyId) throws SystemException {
		...
		DynamicQuery dynamicQuery = DynamicQueryFactoryUtil
				.forClass(de.uhh.l2g.plugins.model.impl.LectureseriesImpl.class, "lect");
		//
		if (Validator.isDigit(keywords)) {
			...
			Criterion likeName = PropertyFactoryUtil.forName("lect.name")
	...
	public List<Lectureseries> getFilteredByInstitutionParentInstitutionTermCategoryCreatorSearchString(
			Long institutionId, Long parentInstitutionId, Long termId, Long categoryId, Long creatorId,
			String searchQuery, Long groupId, Long companyId) {
		return lectureseriesFinder.findFilteredByInstitutionParentInstitutionTermCategoryCreatorSearchString(
				institutionId, parentInstitutionId, termId, categoryId, creatorId, searchQuery, groupId, companyId);
	}
