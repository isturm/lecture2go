...
public class VideoFinderImpl extends VideoFinderBaseImpl implements VideoFinder {

	public static final String RESET_LECTURESERIES_FOR_VIDEOS = VideoFinder.class.getName()
			+ ".resetLectureseriesForVideos";
	public static final String FIND_LATES_OPEN_ACCESS_VIDEO_FOR_LECTURESERIES = VideoFinder.class.getName()
			+ ".findLatestOpenAccessVideoForlectureseries";
	...
	public static final String FIND_VIDEOS_BY_SEARCH_WORD_AND_LECTURESERIESID = VideoFinder.class.getName()
	...
	public List<Video> findVideosByHits(long hits) {
		...
		try {
			...
			@SuppressWarnings("unchecked")
			List<Object[]> l = (List<Object[]>) QueryUtil.list(q, getDialect(),
...
//			qPos.add(lectureseriesId);			
	...
	public Video findVideoBySecureUrl(String surl, boolean mustBeCurrentlyValid) {
		Session session = null;
		...
		try {
			...
			qPos.add(surl + "%");
			@SuppressWarnings("unchecked")
			List<Long> vl = (List<Long>) QueryUtil.list(q, getDialect(),
					com.liferay.portal.kernel.dao.orm.QueryUtil.ALL_POS,
					com.liferay.portal.kernel.dao.orm.QueryUtil.ALL_POS);
			if (mustBeCurrentlyValid) {
				video = VideoLocalServiceUtil.getCurrentlyValidVideo(vl.get(0));
			} else {
