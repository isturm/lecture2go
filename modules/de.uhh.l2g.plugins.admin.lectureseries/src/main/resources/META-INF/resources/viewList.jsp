...
<div class="view list">
    ...
    <div class="col-8">
        <a href="${addURL}" class="add-link">
            <span class="icon-plus"></span> <liferay-ui:message key="add-new-lectureseries"/>
        </a>

        <aui:form action="${lectureseriesSearchURL}" method="post" name="fm">
            <liferay-ui:search-container emptyResultsMessage="no-lectureseries-found" delta="10"
                ...
                <liferay-ui:search-container-row className="de.uhh.l2g.plugins.model.Lectureseries"
                    ...
                    <%
                        ...
                        try {
                            t = TermLocalServiceUtil.getById(lectser.getTermId());
                        } catch (Exception e) {
                        }
                    %>
                            ...
                            <div class="admintile wide">
                                ...
                                <c:if test="${lectser.numberOfVideos > 0}">
                                    <p>
                                        ...
                                        <c:if test="${lectser.numberOfVideos > 1}"><liferay-ui:message
                                                key="video-datasets"/></c:if>
                                        <c:if test="${lectser.numberOfVideos == 1}"><liferay-ui:message
                                                key="video-dataset"/></c:if>
                                    </p>
                                </c:if>
                                <c:if test="${lectser.numberOfVideos == 0}">
                                ...
                                <span class="links">
                                <a href="${lectser.closedAccessURI}" target="blank"><liferay-ui:message
                                        key="lecture-series-closed-access-uri"/></a>&nbsp;|&nbsp;<a
                                        href="${lectser.closedAccessURI}" target="blank"><liferay-ui:message
                                        key="lecture-series-open-access-uri"/></a>
                                    </span>
                            </div>
                            <div class="admintile icons">
                                    ...
                                    <c:otherwise>
                                        ...
                                        <span class="icon-large icon-remove"
                                              onclick="return confirm('<liferay-ui:message
                                                      key="really-delete-question"/>')"></span>
                                        </a>
                                    </c:otherwise>
                                </c:choose>
